  ...
  render() {
      ...
      persons = (
        <div>
          {this.state.persons.map((person, index) => {
            return (
              <ErrorBoundary key={person.id} note={'key always has to be in outermost component'}>
                <Person
                  name={person.name}
                  age={person.age}
                  key={person.id}
    ...
    return (
      <div className="App">
        ...
        <StyledButton
          // style={style}    <-- Radium styling
          alt={this.state.showPersons ? 1 : 0} note={`<-- styled-components styling`}
